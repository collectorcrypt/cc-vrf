    ...
    <main className="flex flex-col gap-16 pb-24 pt-12 sm:pt-20">
      <section className="container-wide flex flex-col gap-6">
        ...
        <p className="max-w-3xl text-lg text-ink-300">
          Each VRF call writes a Light Protocol compressed PDA at a deterministic
          ...
          careless verifier can&rsquo;t be tricked into reading the wrong record.
        </p>
        <p className="max-w-3xl text-base text-ink-400">
          ...
          sub-variant &mdash; same PDA mode, but also persists the 64-byte
          ECVRF beta on chain so other Solana programs can consume the random
          value directly. Same cost as plain registry mode, just more bytes
          per leaf.
        </p>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          <Stat label="Per-call cost (devnet)" value="~$0.0024" sub="single tx" />
          <Stat label="Per 100k calls" value="~$240" sub="extrapolated" />
          <Stat label="RPC requirement" value="Photon" sub="Helius dev plan or equiv." />
        </div>
      </section>
      ...
      <section className="container-wide flex flex-col gap-6">
        ...
        <ul className="space-y-3 text-ink-300">
          <Bullet>
            <strong className="text-ink-50">Public verifiers integrate
            against your VRF.</strong>{" "}
            Anyone who knows{" "}
            <code className="font-mono text-ink-100">
              (authority, memo)
            </code>{" "}
          ...
          <Bullet>
            ...
            Lotteries, public draws, governance &mdash; cases where
            &ldquo;just trust the operator + cryptography&rdquo; isn&rsquo;t
            enough, and you want the chain itself to enforce uniqueness.
          </Bullet>
          <Bullet>
            <strong className="text-ink-50">Audit traceability matters
            more than per-call cost.</strong>{" "}
            Each commit has a permanent compressed-PDA address you can cite in
            a dispute. The Merkle proof of inclusion is constructible by any
      ...
      <section className="container-wide flex flex-col gap-6">
        ...
        <div className="card grid gap-6 sm:grid-cols-2">
          <div>
            ...
            <ul className="space-y-1 text-sm text-ink-300">
              ...
              <li>
                <code className="font-mono">suite</code>: 0x03 (TAI) or 0x04 (ELL2)
              </li>
              <li>
                <code className="font-mono">label</code>: 32-byte identifier
              </li>
              <li>
                <code className="font-mono">frozen</code>, <code className="font-mono">revoked</code>: lifecycle flags
              </li>
            </ul>
          </div>
          <div>
        ...
        <p className="text-sm text-ink-400">
          Both are Light Protocol compressed PDAs &mdash; the chain stores
          only the Merkle root; the actual leaf data lives in the Photon
          indexer.
        </p>
      </section>
      ...
      <section className="container-wide flex flex-col gap-6">
        <h2 className="section-title">
          Cross-program consumption: the <code className="font-mono text-ink-100">+ beta</code> variant
        </h2>
        ...
        <p className="max-w-3xl text-ink-300">
          The <code className="font-mono text-ink-100">commit_proof_with_beta</code>{" "}
          variant stores the full 64-byte beta in the compressed PDA
          alongside the hashes. Any other program can then read the random
          value via a Light SDK CPI &mdash; no off-chain fetch needed.
        </p>
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="text-ink-200">
              ...
              <tr className="border-t border-ink-800">
                ...
                <td className="px-4 py-3 align-top font-mono">~$0.0024</td>
                <td className="px-4 py-3 align-top font-mono">~$0.0024 (same!)</td>
              </tr>
              <tr className="border-t border-ink-800">
                <td className="px-4 py-3 align-top font-medium text-ink-100">
              ...
              <tr className="border-t border-ink-800">
                ...
                <td className="px-4 py-3 align-top">
                  Same &mdash; verifyEndToEnd works identically
                </td>
                <td className="px-4 py-3 align-top">
                  Same &mdash; verifyEndToEnd works identically, plus you can
                  also check stored beta matches vrfProofToHash(proof)
                </td>
              </tr>
              <tr className="border-t border-ink-800">
                <td className="px-4 py-3 align-top font-medium text-ink-100">
                ...
                <td className="px-4 py-3 align-top font-mono">vrf_proof_b</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div className="card border-accent-500/20 bg-accent-500/5">
          <p className="text-sm text-ink-300">
            ...
            storing beta on chain doesn&rsquo;t add cryptographic
            verification &mdash; an on-chain consumer still trusts that the
            operator (whose pk is frozen in{" "}
            <code className="font-mono text-ink-100">VrfAuthority</code>)
            wrote the correct beta. Public auditors can later detect any
            mismatch between the stored beta and{" "}
            <code className="font-mono text-ink-100">
              vrfProofToHash(proof)
            </code>{" "}
            by fetching the proof off-chain and re-running the math.
            Trustless on-chain ECVRF verification would require running RFC
            9381 in BPF (~200k+ CU per call) or an Ed25519-curve precompile
            Solana doesn&rsquo;t have yet.
          </p>
        </div>
      </section>
...
const result = verifyEndToEnd({ pk, alpha, proof, memo, onChainCommit: commit!.onChainCommit });
